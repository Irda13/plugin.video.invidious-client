...
from lib.api_clients.piped import PipedClient
...
class Plugin:
    ...
    def play_video(self, video_id):
        ...
        input_stream_helper = inputstreamhelper.Helper(video.type)
        if not input_stream_helper.check_inputstream():
            raise RuntimeError(
                f"Cannot play video because Inpustream Helper does not support '{video.type}' protocol"
            )
        ...
        play_item.setProperty("inputstream.adaptive.manifest_type", video.type)
    ...
    def search(self, query, page=1):
        ...
        page = int(page)
        client = self._get_client()
        videos = client.search(query=query, region=self._get_setting("region"))
    ...
    def _display_videos(self, videos):
        ...
        for video in videos:
            ...
            url = self._router.build_route("play_video", video_id=video.id)
            items.append((url, list_item, False))
        # add items to directory
        xbmcplugin.addDirectoryItems(self._plugin_handle, items=items)

    def _get_client(self):
        return PipedClient(self._get_setting("instance_url"))
